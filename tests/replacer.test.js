...
describe('replaceWithMetric', () => {
  it('appends metric to feet measurement', () => {
    assert.equal(replaceWithMetric('6.6 feet'), '6.6 feet (2.01 m)');
  });

  it('appends metric to inches measurement', () => {
    assert.equal(replaceWithMetric('36 inches'), '36 inches (91.44 cm)');
  });
  ...
  it('does not modify already-metric text', () => {
    assert.equal(replaceWithMetric('25.4 cm wide'), '25.4 cm wide');
  });
  ...
  it('does not modify prices', () => {
    assert.equal(replaceWithMetric('Only $10 in stock'), 'Only $10 in stock');
  });

  it('does not double-convert when metric already follows in parens', () => {
    assert.equal(replaceWithMetric('6.6FT (2.01 m)'), '6.6FT (2.01 m)');
  });

  it('converts "36 in" with bare in', () => {
    assert.equal(replaceWithMetric('36 in'), '36 in (91.44 cm)');
  });

  it('bare oz no longer matches fluid_oz (will match weight_oz after M2)', () => {
    assert.equal(replaceWithMetric('16 oz'), '16 oz');
  });

  it('bare onzas no longer matches fluid_oz (will match weight_oz after M2)', () => {
    assert.equal(replaceWithMetric('16 onzas'), '16 onzas');
  });

  it('converts large fl oz to liters', () => {
    assert.equal(replaceWithMetric('64 fl oz'), '64 fl oz (1.89 L)');
  });
